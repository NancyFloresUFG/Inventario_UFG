...
def traslados():
    ...
    if request.method == 'POST':
        ...
        if nueva_area and activo['id_area'] is not None and int(nueva_area) != activo['id_area'] or nueva_area and activo['id_area'] is None:
            ...
            cambios.append(f"Area -> {nombre_n}")
        ...
        cambio_ubicacion = any(
            c.startswith("Area") or c.startswith("Responsable") or c.startswith("Uso")
            for c in cambios
        )
        ...
        print(f"DEBUG traslados.py - Usuario en sesión al guardar traslado: '{usuario_sesion}' | id_usuario: {session.get('id_usuario')} | rol: {session.get('rol')}")

        cursor.execute("""
            INSERT INTO movimientos (tipo, id_activo, detalle, motivo, usuario)
            VALUES (%s, %s, %s, %s, %s)
        """, (tipo_mov, id_activo, detalle_str, motivo, usuario_sesion))
