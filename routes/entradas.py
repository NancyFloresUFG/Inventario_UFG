...
def entradas():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"DEBUG entradas.py - Usuario en sesión al guardar activo: '{usuario_sesion}' | id_usuario: {session.get('id_usuario')} | rol: {session.get('rol')}")

            cursor.execute("""
                INSERT INTO movimientos (tipo, id_activo, detalle, motivo, usuario)
                VALUES (%s, LAST_INSERT_ID(), %s, %s, %s)
            """, ('Entrada', detalle, motivo, usuario_sesion))
